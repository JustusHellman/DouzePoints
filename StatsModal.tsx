...
export const StatsModal: React.FC<StatsModalProps> = ({ stats, onClose, onShowInfo, initialTab = 'TOTAL' }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-2xl z-[200] flex items-center justify-center p-4 animate-in fade-in duration-300">
      <div className="bg-[#0a0a1a] border border-white/10 rounded-[2.5rem] p-8 max-w-md w-full relative shadow-3xl overflow-hidden border-t-purple-500/50 max-h-[90vh] overflow-y-auto scrollbar-hide">
        ...
        <div className="flex gap-1 bg-white/5 p-1 rounded-2xl mb-8 overflow-x-auto scrollbar-hide">
          {['TOTAL', GameType.WORD_GAME, GameType.ARTIST_WORD_GAME, GameType.LINKS_GAME, GameType.GUESSER, GameType.ARENA].map(tab => (
            <button 
              ...
              className={`flex-1 py-2.5 px-3 rounded-xl text-[9px] font-black uppercase tracking-tighter transition-all whitespace-nowrap ${activeTab === tab ? 'bg-white text-black shadow-lg' : 'text-gray-500 hover:text-gray-300'}`}
            >
        ...
        <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
          {activeTab === 'TOTAL' ? (
            <div className="space-y-8">
              <div className="text-center p-6 bg-gradient-to-b from-white/5 to-transparent rounded-[2rem] border border-white/5">
                <span className="text-[10px] font-black text-pink-500 uppercase tracking-[0.3em] block mb-2">{t(`ranks.${rankInfo.current.title}`)}</span>
                ...
                {rankInfo.next && (
                  <p className="text-[9px] text-gray-500 font-bold uppercase tracking-widest">{rankInfo.next.threshold - stats.totalPoints} {t('stats.pointsNeeded')} {t(`ranks.${rankInfo.next.title}`)}</p>
                )}
              </div>
        ...
        {/* Action Buttons with hierarchical style and ad clearance */}
        <div className="mt-12 pb-28 flex flex-col gap-3">
            <button 
                onClick={handleShareStats}
                className="w-full bg-white/5 border border-white/10 text-white/70 py-4 rounded-full font-black uppercase text-[10px] tracking-widest hover:bg-white/10 hover:text-white transition-all active:scale-[0.98]"
            >
                {showCopied ? t('common.copied') : (activeTab === 'TOTAL' ? t('common.shareHallOfFame') : t('common.shareCareer'))}
            </button>
            <button 
                onClick={handlePlayNow}
                className="w-full bg-white text-black py-4 rounded-full font-black uppercase text-[10px] tracking-widest hover:scale-[1.02] active:scale-[0.98] transition-all shadow-xl shadow-white/10"
            >
                {t('stats.gotIt')}
            </button>
        </div>
