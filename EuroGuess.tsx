...
const HintBox: React.FC<HintBoxProps> = ({ hint, attempt, idx, isActive, songTitle }) => {
  ...
  return (
    <div 
      className={`rounded-[2rem] md:rounded-[2.5rem] bg-gray-900/50 border-l-[8px] p-4 sm:p-5 md:p-8 shadow-xl relative overflow-hidden transition-all duration-[1500ms] ${
        isActive 
    ...
    >
      ...
      <div className="flex justify-between items-start mb-1.5 md:mb-3 relative z-10">
        <span className="text-white/30 font-black uppercase text-[7px] md:text-[10px] tracking-[0.3em] block">
          {hint.label}
        </span>
        {attempt ? (
          <div className="flex items-center gap-1.5 bg-black/20 px-3 py-1 rounded-full border border-white/5">
            <span className="text-[7px] md:text-[9px] font-black text-gray-500 uppercase tracking-widest truncate max-w-[80px] sm:max-w-[120px] md:max-w-[250px]">
              {attempt}
            </span>
            <div className={`shrink-0 w-3.5 h-3.5 rounded-full flex items-center justify-center text-[7px] font-black text-white ${
          ...
          <span className="text-[7px] md:text-[10px] font-black text-cyan-500/40 uppercase tracking-widest">
            Hint {idx + 1}
          </span>
        )}
      </div>

      <span className="text-base sm:text-lg md:text-2xl font-black tracking-tight text-white/90 break-words relative z-10 leading-tight block">
        {hint.value}
      </span>
    </div>
  );
    ...
    <div className="flex flex-col items-center pt-6 pb-12 px-4 w-full max-w-2xl mx-auto">
      {(!isGameOver || !showModal) && (
        <>
          ...
          {!isGameOver && (
            <div className="w-full flex flex-col gap-6 relative mb-12 z-[150]" ref={searchRef}>
              <input 
                type="text" value={query}
                onChange={(e) => { setQuery(e.target.value); setShowResults(true); }}
                placeholder={t('guesser.searchPlaceholder')}
                className="w-full bg-gray-950 border-2 border-white/5 rounded-[2.5rem] px-8 py-5 md:py-9 font-bold focus:border-cyan-500 outline-none text-center text-lg md:text-2xl shadow-inner text-white transition-all duration-300"
              />
              {showResults && query.length >= 1 && (
                <div className="absolute top-full left-0 right-0 mt-3 bg-[#111122] border border-white/10 rounded-2xl shadow-3xl overflow-hidden z-[160] backdrop-blur-xl max-h-60 overflow-y-auto">
                  {filteredSongs.length > 0 ? filteredSongs.map((s) => (
                    <button key={s.id} onClick={() => handleSelectSong(s)} className="w-full text-left px-6 py-5 transition-colors flex justify-between items-center border-b border-white/5 last:border-0 hover:bg-cyan-500/20">
                  ...
                  )) : <div className="px-6 py-6 text-[10px] md:text-xs font-black text-gray-600 uppercase tracking-widest text-center italic">{t('guesser.noResults')}</div>}
                </div>
              )}
            </div>
          )}
          ...
          {isGameOver && !showModal && (
            <div className="w-full flex justify-center mt-12">
               <button 
                 ...
                 className="bg-white text-black px-14 py-6 rounded-full font-black uppercase text-sm tracking-widest shadow-2xl transition-all hover:scale-105 active:scale-95"
               >
