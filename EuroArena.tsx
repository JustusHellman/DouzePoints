...
const ComparisonBox = ({ label, value, status, arrow, delay }: { label: string, value: string | number, status: 'green' | 'yellow' | 'gray', arrow?: 'up' | 'down', delay: number }) => {
  ...
  return (
    <div className={`relative flex flex-col items-center justify-center p-1.5 md:p-4 rounded-xl md:rounded-2xl border-2 transition-all duration-700 w-full aspect-square text-center transform-gpu ${revealed ? `${colorClasses[status]} scale-100 rotate-0 opacity-100` : 'bg-gray-900 border-white/5 scale-75 opacity-0'}`}>
      <span className="text-[7px] md:text-[11px] font-black uppercase tracking-widest text-white/50 mb-0.5 md:mb-1.5 leading-none italic">{label}</span>
      <div className={`flex items-center gap-0.5 md:gap-1.5 font-black leading-tight w-full justify-center px-0.5`}>
        <span className="text-white drop-shadow-md break-words line-clamp-2 text-[9px] sm:text-[10px] md:text-[16px] leading-tight hyphens-auto">{value}</span>
      ...
      {(!isGameOver || !showModal) && (
        <>
          ...
          {!isGameOver && (
            <div className="w-full relative mb-10 z-[150]" ref={searchRef}>
               <input 
                  ...
                  className="w-full bg-gray-950 border-2 border-white/5 rounded-2xl px-6 py-5 md:py-8 font-bold focus:border-emerald-500 outline-none text-center text-lg md:text-2xl shadow-inner text-white transition-all"
                />
          ...
          <div className="w-full space-y-6">
            {guesses.map((g, idx) => (
              <div key={idx} className="bg-white/5 p-4 md:p-6 rounded-[2rem] md:rounded-[2.5rem] border border-white/5 animate-in slide-in-from-top-6 duration-700 shadow-2xl relative overflow-hidden">
                 ...
                 <div className="flex justify-between items-center mb-5 px-2">
                    <div className="flex flex-col">
                       ...
                       <span className="text-[9px] md:text-xs font-bold text-gray-500 uppercase break-words">{g.artist}</span>
                    </div>
                    <div className="text-[10px] md:text-xs font-black text-emerald-500 px-4 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20 shrink-0">#{guesses.length - idx}</div>
                 </div>
