...
const EuroWordle: React.FC<EuroWordleProps> = ({ onReturn, data, gameType, gameId, title }) => {
  const song = useMemo(() => {
    ...
    const idx = getDailyIndex(data, gameId);
    return data[idx];
  }, [data, gameId]);

  const targetText = useMemo(() => {
    // Corrected GameType property from WORDLE to WORD_GAME
  ...
  const handleShare = () => {
    ...
    alert("Results copied to clipboard! Share the drama!");
  };
  ...
  return (
    <div className="flex flex-col items-center py-6 overflow-hidden w-full max-w-2xl mx-auto">
      ...
      <div className="flex flex-col gap-2 mb-8 items-center px-4 w-full overflow-x-auto scrollbar-hide" aria-label="Game board">
      ...
      {!isGameOver && (
        <div className="w-full px-4 space-y-2 mt-auto" aria-label="Virtual keyboard">
