...
import { syncGameResultToFirestore, safeUpdateUserDoc, canSyncToCloud } from './syncService.ts';
...
export const countTotalPlayed = (stats: GlobalStats): number => {
  const games: (keyof GlobalStats)[] = ['word_game', 'artists', 'links', 'guesser', 'arena', 'refrain'];
  return games.reduce((sum, key) => {
    const s = stats[key] as DetailedStats;
    return sum + (Number(s?.played) || 0);
  }, 0);
};

export const mergeGameBundle = (local: DetailedStats, remote: DetailedStats): DetailedStats => {
  const localPlayed = Number(local?.played) || 0;
  const remotePlayed = Number(remote?.played) || 0;

  // Compare each game mode individually:
  // Whichever environment has more plays for THIS specific game wins its entire bundle.
  // If tied, remote (Cloud) takes authority.
  const dominant = remotePlayed >= localPlayed ? remote : local;
  const secondary = remotePlayed >= localPlayed ? local : remote;

  const result: DetailedStats = {
    played: Number(dominant?.played) || 0,
    wins: Number(dominant?.wins) || 0,
    perfectGames: Number(dominant?.perfectGames) || 0,
    currentStreak: Number(dominant?.currentStreak) || 0,
    maxStreak: Number(dominant?.maxStreak) || 0,
    distribution: [...(dominant?.distribution || [0, 0, 0, 0, 0, 0])],
    lastPlayed: dominant?.lastPlayed || secondary?.lastPlayed || "",
    dailyCompletion: dominant?.dailyCompletion || secondary?.dailyCompletion
  };

  // Today's game exception:
  // If secondary played today's game and dominant has not yet played today,
  // attach today's completion so the player doesn't lose today's solved puzzle.
  const today = getDayString();
  if (secondary?.lastPlayed === today && secondary?.dailyCompletion && result.lastPlayed !== today) {
    result.lastPlayed = today;
    result.dailyCompletion = secondary.dailyCompletion;
  }

  return result;
};

export const mergeDetailedStats = (local: DetailedStats, remote: DetailedStats): DetailedStats => {
  return mergeGameBundle(local, remote);
};

export const mergeGlobalStats = (local: GlobalStats, remote: GlobalStats): GlobalStats => {
  const localTotalPlayed = countTotalPlayed(local);
  const remoteTotalPlayed = countTotalPlayed(remote);

  let dominant: GlobalStats;

  if (remoteTotalPlayed > localTotalPlayed) {
    dominant = remote;
  } else if (localTotalPlayed > remoteTotalPlayed) {
    dominant = local;
  } else {
    // Tiebreaker: compare Total Career Points
    const localPts = Number(local.totalPoints) || 0;
    const remotePts = Number(remote.totalPoints) || 0;
    if (localPts > remotePts) {
      dominant = local;
    } else {
      dominant = remote; // Cloud takes precedence on exact ties
    }
  }

  // Indivisible bundle base: take all stats strictly from the dominant source
  return {
    word_game: { ...dominant.word_game },
    artists: { ...dominant.artists },
    links: { ...dominant.links },
    guesser: { ...dominant.guesser },
    arena: { ...dominant.arena },
    refrain: { ...dominant.refrain },
    totalPoints: Number(dominant.totalPoints) || 0,
    totalDouzePoints: Number(dominant.totalDouzePoints) || 0
  };
};
